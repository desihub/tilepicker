...
mpl.rc('font', size=13)
mpl.rc('axes', prop_cycle=cycler(color=['k', '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']))
...
#ax.plot(delta_midnight, sunaltaz.alt, color='r', ls='-', lw=4, label='Sun')

ax.plot(delta_midnight, moonaltaz.alt, ls='--', lw=4,
        alpha=0.7*m.phase/100 + 0.3,
        label='Moon (phase {:.0f}%)'.format(m.phase))

data = ascii.read('sv1.txt', format='csv')
print(data)
n = len(data)
n = 5
col = mpl.cm.magma(np.linspace(0.1,0.8,n))
lin = ['-', ':', '-.', '--']
...
fig.savefig('tile_airmass.png', dpi=120)
plt.show()
